...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `CMP-EVID-001` | `evidence_core.py` | Client-neutral CSV parsing, datatype/timestamp validation, required-column checks, canonical JSON and SHA-256 integrity mechanics | candidate `SYNNERGYZE_BACKGROUND` **within this Matter** | No VOI contract ID, VOI evidence-ref namespace, LOGIC/provider schema, channel constant or client-specific field dictionary is present | CSV normalization, finite numeric/integer validation, ISO date/time parsing, source hashing, canonical serialization and digest construction are physically separated reusable primitives | Python standard library only; the module does not contain runtime client records | Remains inside MAT-VJRIPL-001. Cross-Matter export/package publication still requires a separate rights basis and provenance record | `ENGINEERING_CLASSIFICATION_ONLY`; `LEGAL_TITLE_UNRESOLVED`; `REUSE_REQUIRES_RIGHTS_BASIS`; `NO_PRODUCTION_AUTHORITY` | Physical internal separation complete under Issue #19; no external extraction authorized |
| `CMP-EVID-001A` | `voi_evidence_adapter.py` | VOI inventory evidence contract/profile implementation using `evidence_core.py` | `VOI_FOREGROUND` with dependency on candidate `SYNNERGYZE_BACKGROUND` | `VOI-INVENTORY-EVIDENCE-001`, source names, required fields, demand-window semantics, VOI evidence refs, snapshot construction and business validation remain in the Matter-specific layer | Delegates generic parsing/validation/canonicalization/hashing to `evidence_core.py` | Runtime source rows and generated evidence instances are `VOI_DATA_KNOWLEDGE` | Do not export VOI schema, refs, data or business rules with the generic core; contract behavior is frozen by regression/golden-digest tests | `ENGINEERING_CLASSIFICATION_ONLY`; `LEGAL_TITLE_UNRESOLVED`; `VOI_DATA_EXPORT_RESTRICTED`; `NO_PRODUCTION_AUTHORITY` | Keep adapter/profile client-specific; later profile-data extraction may be considered separately |
| `CMP-EVID-002` | `VOI-INVENTORY-EVIDENCE-001` return structure in `voi_evidence_adapter.py` | VOI inventory evidence contract | `VOI_FOREGROUND` + `VOI_DATA_KNOWLEDGE` | Contract ID, snapshot fields, demand-window semantics, source/evidence naming and current business meaning are specific to VOI | Digest/canonical-payload convention is now delegated to `evidence_core.py` without moving the VOI contract | Payload instances contain VOI operational evidence | Do not copy payload/schema into another client Matter without explicit rights basis; generalized integrity mechanics remain separately classified | `VOI_DATA_EXPORT_RESTRICTED`; `LEGAL_TITLE_UNRESOLVED` | Keep generic evidence-envelope mechanics separate from the VOI contract/profile |
...
| `CMP-EVID-006` | `tests/test_voi_evidence_adapter.py`, `tests/test_evidence_core_boundary.py`, `.github/workflows/voi-evidence-bridge.yml` | Regression, neutrality and CI gates for evidence contract/core boundary | `VOI_FOREGROUND` + candidate `SYNNERGYZE_BACKGROUND` verification + `THIRD_PARTY_PROTOTYPE` tooling | VOI edge cases, frozen contract digest and expected contract behavior remain client/application-specific | Neutrality tests prove `evidence_core.py` contains no VOI/LOGIC/Easycom/Myntra/Flipkart/Centro constants; CI compiles the core explicitly | GitHub Actions marketplace actions and Python packages are third-party dependencies; fixtures are synthetic | Reuse test techniques generically; VOI golden digest/fixtures and business semantics do not transfer automatically | `ENGINEERING_CLASSIFICATION_ONLY`; `NO_PRODUCTION_AUTHORITY` | Keep generic-core neutrality tests separate from VOI contract regressions |
...
The evidence boundary is now physically represented inside this Matter:

```text
evidence_core.py
  canonicalize / validate / parse / fingerprint / hash
                    ↓
voi_evidence_adapter.py
  VOI contract / field dictionary / source semantics / evidence-ref namespace
                    ↓
VOI evidence instances
  inventory / sales / product master / open orders / derived snapshots
```

`evidence_core.py` is a candidate `SYNNERGYZE_BACKGROUND` mechanism **within MAT-VJRIPL-001**. `voi_evidence_adapter.py` retains the `VOI_FOREGROUND` contract and business semantics; instantiated records remain `VOI_DATA_KNOWLEDGE`.

Issue #19 does not export the core to another Matter and does not establish a licence or legal title. The compatibility boundary is enforced by existing evidence regressions plus a frozen minimal bundle digest and client-neutrality tests.
...
1. separately governed export/package decision for `evidence_core.py` only if a future receiving Matter and rights basis are identified;
...
No cross-Matter refactor, code movement or licence is authorized by this register itself.
