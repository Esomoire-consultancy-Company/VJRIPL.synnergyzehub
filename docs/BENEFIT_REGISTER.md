...
| BEN-VOI-014 | Physical evidence-core / VOI-profile separation | Preserves the existing VOI inventory-evidence contract while physically separating client-neutral parsing, validation, canonicalization and hashing from VOI-specific contract IDs, field mappings, demand semantics and evidence-ref naming. This makes the Matter's attributable client layer auditable and reduces future accidental leakage of VOI configuration/data into reusable code. | Issue #19, PR #20, `evidence_core.py`, `voi_evidence_adapter.py`, `tests/test_evidence_core_boundary.py`, updated component register | Candidate `SYNNERGYZE_BACKGROUND` mechanics remain inside this Matter; VOI adapter/profile remains `VOI_FOREGROUND`; instantiated evidence remains `VOI_DATA_KNOWLEDGE`; legal title/export rights unresolved | Internal refactor only; no cross-Matter export, licence, publication, live provider credentials or production-write authority | `MATTER_CAPABILITY` |
...
Issue #19 makes the first of those boundaries physical in code, but the internal extraction does not itself create a cross-Matter reuse right or licence.
