...
""")
            ...
            red = next(item for item in bundle["snapshots"] if item["sku"] == "VOI-RED-M")
            blue = next(item for item in bundle["snapshots"] if item["sku"] == "VOI-BLUE-L")
            ...
            self.assertIn("overdue open order excluded", bundle["warnings"][0])
